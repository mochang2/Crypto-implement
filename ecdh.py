...
def GenPrivateKey(nbit):
    return str(bin(random.getrandbits(nbit)))[2:], str(bin(random.getrandbits(nbit)))[2:]
...
def ScalarMul(NList):
    ...
    for N in NList:
        temp = P
        for bit in N[1:]:
            temp = ec.addition(temp, temp)
            if bit == "1":
                temp = ec.addition(temp, P)
        li.append(temp)
    return li[1], li[0]
...
if __name__ == "__main__":
    ...
    Na, Nb = GenPrivateKey(160)
    print("Private Key Na:", Na)
    print("Private Key Nb:", Nb)

    Qb, Qa = ScalarMul([Na, Nb])
    print("Public Key Qb:", Qb)
    print("Public Key Qa:", Qa)

    Qab, Qba = ScalarMul([Qb, Qa])
    print("Public Key Qab:", Qab)
    print("Public Key Qba:", Qba)
